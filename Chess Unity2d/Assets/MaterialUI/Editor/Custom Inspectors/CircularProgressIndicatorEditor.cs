...
namespace MaterialUI
{
    ...
    public class CircularProgressIndicatorEditor : MaterialBaseEditor
    {
        ...
        {
            var circleRectTransformValue = m_CircleRectTransform.objectReferenceValue as RectTransform;

            if (circleRectTransformValue == null) return false;
            if (circleRectTransformValue.childCount == 0) return false;

            return InspectorFields.GraphicColorField("Circle",
                circleRectTransformValue.GetChild(0).GetComponent<Graphic>());
        }
