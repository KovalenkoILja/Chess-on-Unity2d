    ...
    public class MaterialInputFieldEditor : MaterialBaseEditor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUI.EndChangeCheck())
                ((MaterialInputField) m_Interactable.serializedObject.targetObject).interactable =
                    m_Interactable.boolValue;

            if (m_HintTextTransform.objectReferenceValue != null) EditorGUILayout.PropertyField(m_HintText);
            ...
            using (new GUILayout.HorizontalScope())
            {
                ...
                if (m_FloatingHint.boolValue)
                {
                    EditorGUILayout.LabelField("Font Size",
                        GUILayout.Width(GUI.skin.label.CalcSize(new GUIContent("Font Size")).x));
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    if (!m_MatchInputFieldCharacterLimit.boolValue)
                    {
                        ...
                        EditorGUILayout.LabelField("Limit",
                            GUILayout.Width(GUI.skin.label.CalcSize(new GUIContent("Limit")).x + 10));
                        EditorGUI.indentLevel++;
            ...
            using (new GUILayout.HorizontalScope())
            {
                ...
                if (m_ManualPreferredWidth.boolValue) EditorGUILayout.PropertyField(m_ManualSizeX, new GUIContent(""));
            }
