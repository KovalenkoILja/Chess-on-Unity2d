...
namespace MaterialUI
{
    ...
    {
        private readonly string[] m_AllowedValueTypes = {"Single", "Int", "Vector2", "Vector3", "Rect", "Color"};
        ...
        private void EnforceListLengths()
        {
            ...
            if (m_ScriptTarget.tweens.Count <= 0) m_ScriptTarget.tweens.Add(new EasyTween.EasyTweenObject());

            for (var i = 0; i < m_ScriptTarget.tweens.Count; i++)
        ...
        private string[] FindVariableStrings(GameObject targetGameObject, string targetComponent)
        {
            ...
            if (targetGameObject)
                if (targetGameObject.GetComponent(targetComponent))
                {
                    ...
                    {
                        var propertyType = properties[i].PropertyType.Name;

                        for (var j = 0; j < m_AllowedValueTypes.Length; j++)
                    ...
                    {
                        var fieldType = fields[i].FieldType.Name;
        ...
        {
            var thisMenu = new GenericMenu();
            var componentStrings = FindComponentStrings(targetGameObject);

            for (var i = 0; i < componentStrings.Length; i++)
            {
                var variableStrings = FindVariableStrings(targetGameObject, componentStrings[i]);

                for (var j = 0; j < variableStrings.Length; j++)
                    thisMenu.AddItem(new GUIContent(componentStrings[i] + "/" + variableStrings[j]), false,
                        VariableMenuCallback, (object) componentStrings[i] + "/" + variableStrings[j]);
            }
        ...
        {
            var thisMenu = new GenericMenu();
            var componentStrings = FindComponentStrings(tweenObject.callbackGameObject);

            for (var i = 0; i < componentStrings.Length; i++)
            {
                var methodStrings = FindMethodStrings(tweenObject.callbackGameObject, componentStrings[i]);

                for (var j = 0; j < methodStrings.Length; j++)
                    thisMenu.AddItem(new GUIContent(componentStrings[i] + "/" + methodStrings[j]), false,
                        MethodMenuCallback, (object) componentStrings[i] + "/" + methodStrings[j]);
            }
        ...
        public override void OnInspectorGUI()
        {
            ...
            for (var i = 0; i < m_ScriptTarget.tweens.Count; i++)
            {
                var tweenObject = m_ScriptTarget.tweens[i];
        ...
        private void ConfigureTweenObject(ref EasyTween.EasyTweenObject tweenObject, int tweenIndex)
        {
            ...
            if (tweenObject.optionsVisible)
            {
                using (new GUILayout.VerticalScope("Box"))
                {
                    ...
                    using (new GUILayout.HorizontalScope())
                    {
                        tweenObject.targetGameObject = (GameObject) EditorGUILayout.ObjectField("Target GameObject",
                            tweenObject.targetGameObject, typeof(GameObject), true);
                    ...
                    tweenObject.tweenType =
                        (Tween.TweenType) EditorGUILayout.EnumPopup("Tween type", tweenObject.tweenType);
                    ...
                    if (tweenObject.tweenType == Tween.TweenType.Custom)
                    {
                        ...
                        if (tweenObject.customCurve.keys[0].time != 0f ||
                            tweenObject.customCurve.keys[tweenObject.customCurve.keys.Length - 1].time != 1f)
                            tweenObject.customCurve = new AnimationCurve(new Keyframe(0f, 0f), new Keyframe(1f, 1f));
                    }
                    ...
                    using (new GUILayout.HorizontalScope())
                    {
                        if (tweenObject.hasCallback)
                            tweenObject.callbackGameObject =
                                (GameObject) EditorGUILayout.ObjectField("GameObject", tweenObject.callbackGameObject,
                    ...
                    if (tweenObject.hasCallback && tweenObject.callbackGameObject != null)
                        using (new GUILayout.HorizontalScope())
                        {
                            if (tweenObject.hasCallback)
                            {
                                ...
                                if (GUILayout.Button(tweenObject.callbackComponentName + "/" + tweenObject.callbackName,
                    ...
                    if (tweenObject.targetGameObject)
                    {
                        ...
                        if (tweenObject.subOptionsVisible)
                            for (var i = 0; i < tweenObject.subTweens.Count; i++)
                            {
                                var subObject = tweenObject.subTweens[i];
        ...
        private void ConfgureSubObject(ref EasyTween.EasyTweenSubObject subObject, int tweenIndex, int subIndex)
        {
            using (new GUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button(subObject.targetComponentName + "/" + subObject.targetVariableName,
                    GUI.skin.GetStyle("Popup")))
                {
                    m_TempTweenIndex = tweenIndex;
                    m_TempSubTweenIndex = subIndex;
                    VariableMenu(m_ScriptTarget.tweens[m_TempTweenIndex].targetGameObject,
                        m_ScriptTarget.tweens[m_TempTweenIndex]).ShowAsContext();
                    return;
                }
            ...
            if (subObject.targetComponent != null) ConfigureTargetValue(ref subObject, tweenIndex, subIndex);
        ...
        private void ConfigureTargetValue(ref EasyTween.EasyTweenSubObject subObject, int tweenIndex, int subIndex)
        {
            ...
            if (subObject.variableType == "Int32" || subObject.variableType == "Single") subStrings = new[] {"value"};
            if (subObject.variableType == "Vector2") subStrings = new[] {"X", "Y"};
            if (subObject.variableType == "Vector3") subStrings = new[] {"X", "Y", "Z"};
            if (subObject.variableType == "Vector4") subStrings = new[] {"X", "Y", "Z", "W"};
            if (subObject.variableType == "Rect") subStrings = new[] {"X", "Y", "Width", "Height"};
            if (subObject.variableType == "Color") subStrings = new[] {"R", "G", "B", "A"};
            ...
            if (GUILayout.Button("Copy from gameObject", GUILayout.Width(CalcWidth("Copy from gameObject") + 8f)))
            {
                ...
                {
                    var component = m_ScriptTarget.GetComponent<EasyTween>().tweens[tweenIndex].targetGameObject
                        .GetComponent(
                            m_ScriptTarget.GetComponent<EasyTween>().tweens[tweenIndex].subTweens[subIndex]
                                .targetComponentName);
            ...
            if (GUILayout.Button("Apply to gameObject", GUILayout.Width(CalcWidth("Apply to gameObject") + 8f)))
            {
                if (subObject.isProperty)
                {
                    ...
                    component.GetType().GetProperty(variableName).SetValue(component,
                        Vector4ToObject(subObject.variableType, subObject.targetValue), null);
                }
                else
                {
                    var component = m_ScriptTarget.GetComponent<EasyTween>().tweens[tweenIndex].targetGameObject
                    ...
                    component.GetType().GetField(variableName).SetValue(component,
                        Vector4ToObject(subObject.variableType, subObject.targetValue));
                }
            }

            for (var i = 0; i < subObject.targetValueLength; i++)
                using (new GUILayout.HorizontalScope())
                {
                    subObject.modifyParameters[i] =
                        GUILayout.Toggle(subObject.modifyParameters[i], "Tween " + subStrings[i]);
        ...
        {
            var theString = (string) item;

            var stringArray = theString.Split('/');

            var tweenObject = m_ScriptTarget.tweens[m_TempTweenIndex];
            var subObject = m_ScriptTarget.tweens[m_TempTweenIndex].subTweens[m_TempSubTweenIndex];
            ...
            if (subObject.targetPropertyInfo != null)
            {
                ...
                subObject.variableType = subObject.targetPropertyInfo.GetValue(subObject.targetComponent, null)
                    .GetType().Name;
            ...
            if (subObject.variableType == "Vector2") subObject.targetValueLength = 2;
            if (subObject.variableType == "Vector3") subObject.targetValueLength = 3;
            if (subObject.variableType == "Vector4" || subObject.variableType == "Rect" ||
                subObject.variableType == "Color") subObject.targetValueLength = 4;
        ...
        private Vector4 ObjectToVector4(string valueType, object value)
        {
            switch (valueType)
            {
                ...
                case "Vector2":
                    var tempVector2 = (Vector2) value;
                    return new Vector4(tempVector2.x, tempVector2.y, 0f, 0f);

                case "Vector3":
                    var tempVector3 = (Vector3) value;
