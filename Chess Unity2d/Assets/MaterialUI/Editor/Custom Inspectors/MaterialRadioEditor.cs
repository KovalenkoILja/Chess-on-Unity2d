...
namespace MaterialUI
{
    ...
    internal class MaterialRadioEditor : MaterialToggleBaseEditor
    {
        ...
        {
            m_IsControllingChildren = m_Toggle.GetComponentInParent<MaterialRadioGroup>() &&
                                      m_Toggle.GetComponentInParent<MaterialRadioGroup>().isControllingChildren;
            if (m_IsControllingChildren)
                EditorGUILayout.HelpBox("Some options are controlled by parent MaterialRadioGroup",
                    MessageType.Warning);
