...
namespace MaterialUI
{
    ...
    public class ImageDataPropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var labelWidth = GUI.skin.GetStyle("Label").CalcSize(label).x;

            var i = EditorGUI.PrefixLabel(new Rect(pos.x, pos.y, labelWidth, pos.height), label).width;
            ...
            if (m_ImageDataType.enumValueIndex == 1)
            {
                ...
                var code = m_VectorImageData.FindPropertyRelative("m_Glyph.m_Unicode");
                var name = m_VectorImageData.FindPropertyRelative("m_Glyph.m_Name");
                var font = m_VectorImageData.FindPropertyRelative("m_Font");
                var iconStyle = new GUIStyle {font = (Font) font.objectReferenceValue};
                ...
                if (GUI.Button(
                    new Rect(pos.x + pos.width - 1 - clearButtonPadding, pos.y, clearButtonWidth, pos.height),
                    IconDecoder.Decode(@"\ue14c"),
                    new GUIStyle
