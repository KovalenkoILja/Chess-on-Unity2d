...
namespace MaterialUI
{
    ...
    internal class DropdownOptionListDrawer : PropertyDrawer
    {
        ...
        private float m_ImageTypeHeight;
        ...
        private void Init(SerializedProperty property)
        {
            ...
            var array = property.FindPropertyRelative("m_Options");
        ...
        private void DrawOptionData(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var itemText = itemData.FindPropertyRelative("m_Text");

            var itemSprite = itemData.FindPropertyRelative("m_ImageData.m_Sprite");

            var itemCode = itemData.FindPropertyRelative("m_ImageData.m_VectorImageData.m_Glyph.m_Unicode");
            var itemName = itemData.FindPropertyRelative("m_ImageData.m_VectorImageData.m_Glyph.m_Name");
            ...
            var itemImageType = itemData.FindPropertyRelative("m_ImageData.m_ImageDataType");
            ...
            EditorGUI.PropertyField(new Rect(rect.x + offsetH, rect.y, rect.width / 3 - offsetH, rect.height), itemText,
                GUIContent.none);
            ...
            {
                EditorGUI.PropertyField(new Rect(rect.x + offsetH, rect.y, rect.width - offsetH, rect.height),
                    itemSprite, GUIContent.none);
            }
            else
            {
                if (!string.IsNullOrEmpty(itemName.stringValue))
                {
                    EditorGUI.LabelField(new Rect(rect.x + offsetH, rect.y, 16, rect.height),
                        IconDecoder.Decode(itemCode.stringValue), iconStyle);
                    offsetH += 16;
                    EditorGUI.LabelField(new Rect(rect.x + offsetH, rect.y, rect.width - offsetH - 80, rect.height),
                ...
                {
                    var optionDataListContainer = itemData.serializedObject.targetObject as IOptionDataListContainer;
                    VectorImagePickerWindow.Show(
                        optionDataListContainer.optionDataList.options[index].imageData.vectorImageData,
                        itemData.serializedObject.targetObject);
                }

                if (GUI.Button(new Rect(rect.width + 16, rect.y, 18, rect.height), IconDecoder.Decode(@"\ue14c"),
                    new GUIStyle
                        {font = VectorImageManager.GetIconFont(VectorImageManager.materialDesignIconsFontName)}))
                {
                    var optionDataListContainer = itemData.serializedObject.targetObject as IOptionDataListContainer;
                    var data = optionDataListContainer.optionDataList.options[index].imageData.vectorImageData;
                    data.font = null;
                    data.glyph = null;
