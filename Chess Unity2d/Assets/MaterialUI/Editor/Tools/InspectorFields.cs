...
namespace MaterialUI
{
    ...
    public class DisabledScope : GUI.Scope
    {
        ...
        private readonly bool m_PreviousSetting;
    ...
    public static class InspectorFields
    {
        ...
        ///     If true the property including children is drawn; otherwise only the control itself (such
        ///     as only a foldout but nothing below it).
        /// </param>
        ...
        /// <param name="ignoreMultiple">
        ///     If false, method returns whether multiple GameObjects are selected. If true, returns
        ///     false.
        /// </param>
        ...
        public static void PropertyField<T>(T objectToDo, Action fieldAction,
            bool disableIfMultipleObjectsSelected = true) where T : Object
        {
        ...
        /// <param name="disableIfMultipleObjectsSelected">Is this field drawn disabled if multiple GameObjects are selected?</param>
        public static void PropertyField<T>(T objectToDo, Action fieldAction, Action onChangeAction,
            Action onNoChangeAction, bool disableIfMultipleObjectsSelected = false) where T : Object
        {
        ...
        /// <param name="fieldFunc">
        ///     This func will contain the field action and return its 'output' value eg:
        ///     EditorGUILayout.TextField.
        /// </param>
        /// <param name="onChangeAction">
        ///     This action is called when the inspector control drawn in fieldAction is changed. Set the
        ///     multiple objects' values in here.
        /// </param>
        public static void PropertyMultiField<T1, T2>(T1[] objectsToDo, Func<T2> fieldFunc, Action<T2> onChangeAction)
            where T1 : Object
        {
        ...
        /// <param name="onNoChangeAction">This action is called if the control drawn in fieldAction is not changed.</param>
        public static void PropertyMultiField<T1, T2>(T1[] objectsToDo, Func<T2> fieldFunc, Action<T2> onChangeAction,
            Action onNoChangeAction) where T1 : Object
        {
            for (var i = 0; i < objectsToDo.Length; i++) Undo.RecordObject(objectsToDo[i], "edit value");
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                for (var i = 0; i < objectsToDo.Length; i++) EditorUtility.SetDirty(objectsToDo[i]);
            }
            else
            {
                for (var i = 0; i < objectsToDo.Length; i++) Undo.ClearUndo(objectsToDo[i]);
        ...
        /// <summary>
        ///     Alternative inspector control (eg: EditorGUILayout.SpriteField) for
        ///     Image.sprite/VectorImage.vectorImageData/Text.text. Supports undo/redo.
        /// </summary>
        ...
        public static bool GraphicField(string label, Graphic graphic, bool disableIfMultipleObjectsSelected = true)
        {
            using (new DisabledScope(TooManySelected(!disableIfMultipleObjectsSelected)))
            {
                ...
                var vectorImage = graphic as VectorImage;
                if (vectorImage != null)
                {
        ...
        /// <summary>
        ///     Alternative inspector control (eg: EditorGUILayout.SpriteField) for
        ///     Image.sprite/VectorImage.vectorImageData/Text.text. Supports undo/redo and multiple selected objects.
        /// </summary>
        /// <param name="label">The label to appear in the inspector control.</param>
        ...
        ///     The Graphic to use for the 'current' values in the control if the Graphic values are
        ///     different.
        /// </param>
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool GraphicMultiField(string label, Func<GameObject, Graphic> getReferenceFunc,
            Graphic visualReference = null)
        {
        ...
        {
            if (visualReference == null) visualReference = graphics[0];

            var image = visualReference as Image;
            if (image != null)
            {
                ImageMultiField(label, graphics.Select(graphic => graphic as Image).ToArray(),
                    visualReference as Image);
                return true;
            }

            var vectorImage = visualReference as VectorImage;
            if (vectorImage != null)
            {
                VectorImageMultiField(label, graphics.Select(graphic => graphic as VectorImage).ToArray(),
                    visualReference as VectorImage);
                return true;
            }

            var text = visualReference as Text;
            if (text != null)
            {
        ...
        {
            PropertyField(text, () => text.text = EditorGUILayout.TextField(label, text.text),
                disableIfMultipleObjectsSelected);
            return true;
        }

        /// <summary>
        ///     Alternative inspector control (eg: EditorGUILayout.TextField) for Text.text. Supports undo/redo and multiple
        ///     selected objects.
        /// </summary>
        /// <param name="label">The label to appear in the inspector control.</param>
        ...
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool TextMultiField(string label, Func<GameObject, Text> getReferenceFunc,
            Text visualReference = null)
        {
        ...
        public static bool TextMultiField(string label, Text[] texts, Text visualReference = null)
        {
            ...
            if (visualReference == null) visualReference = texts.ToList().First(text => text != null);

            Action<string> textOnChangeAction = text =>
            {
                for (var i = 0; i < texts.Length; i++) texts[i].text = text;
            };
        ...
        public static bool ImageField(string label, Image image, bool disableIfMultipleObjectsSelected = true)
        {
            using (new DisabledScope(TooManySelected(!disableIfMultipleObjectsSelected)))
            {
                ...
                EditorGUILayout.LabelField(GUIContent.none,
                    new GUIStyle {normal = {background = (Texture2D) image.mainTexture}},
                    GUILayout.Width(spriteIconSize), GUILayout.Height(spriteIconSize));
        ...
        /// <summary>
        ///     Alternative inspector control (eg: EditorGUILayout.SpriteField) for Image.sprite. Supports undo/redo and
        ///     multiple selected objects.
        /// </summary>
        /// <param name="label">The label to appear in the inspector control.</param>
        /// <param name="getReferenceFunc">The function used to get each Image reference from each selected GameObject.</param>
        /// <param name="visualReference">
        ///     The Image to use for the 'current' values in the control if the Image values are
        ///     different.
        /// </param>
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool ImageMultiField(string label, Func<GameObject, Image> getReferenceFunc,
            Image visualReference = null)
        {
        ...
        public static bool ImageMultiField(string label, Image[] images, Image visualReference = null)
        {
            ...
            if (visualReference == null) visualReference = images.ToList().First(image => image != null);

            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUILayout.PrefixLabel(label);
                EditorGUILayout.LabelField(GUIContent.none,
                    new GUIStyle {normal = {background = (Texture2D) visualReference.mainTexture}},
                    GUILayout.Width(spriteIconSize), GUILayout.Height(spriteIconSize));
            ...
            Action onIconPickAction = null, bool disableIfMultipleObjectsSelected = true)
        {
            using (new DisabledScope(TooManySelected(!disableIfMultipleObjectsSelected)))
            {
                ...
                var iconStyle = new GUIStyle {font = font, fontSize = vectorIconSize};
            ...
            Object[] objectsToUndo, Action onIconPickAction = null, VectorImageData visualReference = null)
        {
            ...
            if (visualReference == null) visualReference = targetDatas.ToList().First(data => data != null);
        ...
        /// <summary>
        ///     Inspector control (eg: EditorGUILayout.SpriteField) for Image.sprite/VectorImage.vectorImageData/Text.text and
        ///     .color. Supports undo/redo.
        /// </summary>
        /// <param name="label">The label to appear in the inspector control.</param>
        /// <param name="graphic">The graphic to modify.</param>
        /// <param name="disableIfMultipleObjectsSelected">Is this field drawn disabled if multiple GameObjects are selected?</param>
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool GraphicColorField(string label, Graphic graphic,
            bool disableIfMultipleObjectsSelected = true)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                PropertyField(graphic,
                    () => graphic.color = EditorGUILayout.ColorField(graphic.color,
                        GUILayout.MinWidth(colorFieldMinWidth), GUILayout.MaxWidth(colorFieldMaxWidth)),
                    disableIfMultipleObjectsSelected);
            }
        ...
        /// <summary>
        ///     Inspector control (eg: EditorGUILayout.SpriteField) for Image.sprite/VectorImage.vectorImageData/Text.text and
        ///     .color. Supports undo/redo and multiple selected objects.
        /// </summary>
        /// <param name="label">The label to appear in the inspector control.</param>
        /// <param name="getReferenceFunc">The function used to get each Graphic reference from each selected GameObject.</param>
        /// <param name="visualReference">
        ...
        /// </param>
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool GraphicColorMultiField(string label, Func<GameObject, Graphic> getReferenceFunc,
            Graphic visualReference = null)
        {
            return GraphicColorMultiField(label, Selection.gameObjects.Select(getReferenceFunc).ToArray(),
                visualReference);
        }
        ...
        public static bool GraphicColorMultiField(string label, Graphic[] graphics, Graphic visualReference = null)
        {
            ...
            if (visualReference == null) visualReference = graphics.ToList().First(graphic => graphic != null);

            Action<Color> onChangeAction = color =>
            {
                for (var i = 0; i < graphics.Length; i++) graphics[i].color = color;
            };

            using (new EditorGUILayout.HorizontalScope())
            {
                using (new DisabledScope(!graphics.ToList()
                    .TrueForAll(graphic => graphic.GetType() == visualReference.GetType())))
                {
                    GraphicMultiField(label, graphics, visualReference);
                }

                PropertyMultiField(graphics,
                    () => EditorGUILayout.ColorField(visualReference.color, GUILayout.MinWidth(colorFieldMinWidth),
                        GUILayout.MaxWidth(colorFieldMaxWidth)), onChangeAction);
            }
        ...
        public static bool MaterialButtonField(MaterialButton materialButton, bool showBox = true)
        {
            ...
            if (materialButton.text == null && materialButton.icon == null &&
                materialButton.backgroundImage == null) return false;
            ...
            using (showBox ? new EditorGUILayout.VerticalScope("Box") : new EditorGUILayout.VerticalScope())
            {
                ...
                if (GraphicColorField(nameLabel + "Background", materialButton.backgroundImage)) result = true;
            }
