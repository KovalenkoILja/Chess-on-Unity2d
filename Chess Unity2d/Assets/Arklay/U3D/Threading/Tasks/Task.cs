...
namespace U3D.Threading.Tasks
{
    ...
    public class Task
    {
        ...
        private TState __state = TState.Created;

        private List<Action<Task>> m_continuationActions = new List<Action<Task>>();
                            ...
                            case TState.Created:
                                if (__state != TState.Created)
                                    throw new InvalidOperationException(
                                        string.Format("Invalid state transition; {0} -> {1}", __state, value));
                                break;
                            ...
                            case TState.Faulted:
                                if (__state != TState.Running)
                                    throw new InvalidOperationException(
                                        string.Format("Invalid state transition; {0} -> {1}", __state, value));
                                __state = value;
                                var aux = m_continuationActions;
                                m_continuationActions = new List<Action<Task>>();
                                foreach (var a in aux) a(this);
                                break;
                            default:
        ...
        protected Task()
        {
            Dispatcher.Initialize(); // just in case...
        }
        ...
        /// <summary>
        ///     Gets the <see cref="T:System.AggregateException">Exception</see> that caused the
        ///     <see
        ///         cref="Task">
        ///         Task
        ///     </see>
        ...
        public bool IsCompleted =>
            m_state == TState.Successful || m_state == TState.Aborted || m_state == TState.Faulted;
        ...
        /// </remarks>
        public bool IsFaulted => m_state == TState.Faulted;
        ...
        public bool IsAborted => m_state == TState.Aborted;

        #region default thread pool methods

#if !UNITY_WEBGL
        private static ThreadPool __defaultThreadPool;
        /// <summary>
        ...
        /// <summary>
        ///     Queues the specified work to run on the ThreadPool and returns a Task handle for that work, the action is
        ///     guaranteed to be executed in the main thread
        /// </summary>
...
#else
        private ThreadPool.PooledThread m_runThread;
        ...
        private readonly Queue<Action<Task>> m_whenDone = new Queue<Action<Task>>();
        private readonly object m_whenDoneSync = new object();
