...
namespace U3D.Threading
{
...
#else
        private System.Threading.Thread m_impl;
    ...
    {
        private readonly List<Thread> m_currentThreads = new List<Thread>();
        private readonly Queue<PooledThread> m_queuedActions = new Queue<PooledThread>();
        ...
        /// </summary>
        public int availableThreads => maxThreads - m_currentThreads.Count;
        ...
        {
            var pt = new PooledThread {action = a};
            lock (m_sync)
            {
                if (availableThreads > 0)
                {
                    ...
                    Thread th = new System.Threading.Thread(_th =>
                    {
                        try
                        {
                            a();
        ...
        private void ThreadWillFinish(Thread th)
        {
            Action a = null;
            lock (m_sync)
            {
                ...
                {
                    var pt = m_queuedActions.Dequeue();
                    if (pt.aborted)
                    {
                        ThreadWillFinish(th);
                    }
        ...
        public class PooledThread
        {
            ...
            public bool isEnqueued => thread == null;
